...
def test_runtime_snapshot_tracks_latest_prompt_per_history_key() -> None:
    ...
    state.record_prompt_snapshot(PromptSnapshot(
        history_key="alice",
        person="alice",
...
def test_runtime_snapshot_exposes_recent_skill_and_consolidation_events() -> None:
    ...
    state.record_skill_activation(SkillActivationEvent(
        skill_name="weather",
        person="alice",
        reason="read_skill",
        tool_name=None,
    ...
    state.record_skill_verification(SkillVerificationReport(
        skill_name="weather",
        owner="household",
        scope="household",
        source="skill_create",
    ...
    state.record_consolidation(ConsolidationEvent(
        history_key="alice",
        person="alice",
        status="succeeded",
        reason="ok",
